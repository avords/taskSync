...
public class SendTaskSyncAction {
	private Logger logger = Logger.getLogger(SendTaskSyncAction.class);
	...
	public static void main(String[] args) {
		SendTaskSyncAction action = new SendTaskSyncAction();
		action.execute();
...
//		返回值：{"returnCode":"200","list":[{"total":"7","succ":"28","arriveFail":"0","arriveSucc":"28","status":"4","begintime":"2016-01-19 11:03:49","taskType":"1","nmid":"22441982192","productId":"1","allnumber":"28","createtime":"2016-01-19 11:03:49","id":"22451346189","fail":"0","imContent":"","description":"","netmessageType":"1","userName":"SSD"}],"dataCount":1}
		...
		String sql = "select t.*, s.sp_code from (select distinct portal_task_id id, task_id, sp_id from im_portal_task where task_id in(select * from (select id from im_send_task_summary t where t.begintime>sysdate-"+SystemMessage.getString("limitDay")+" and t.allnumber>nvl(succ,0)+nvl(fail,0) order by t.begintime desc) where rownum<=10000)) t left join im_sp_info s on t.sp_id = s.id ";
